...
repo의 `skills/` 아래 skill을 설치하며 선택형 `cadence-laya`도 포함한다. 일부만 설치하려면 `--skill using-cadence`처럼 선택한다. Laya 연결만 추가하려면 `--skill cadence-laya`를 사용한다.

`cadence-laya` 설치는 런타임·모델 준비나 활성화와 다르다. [설정 안내](skills/cadence-laya/references/setup.md)에 따라 별도로 준비하고 진단한다. 현재는 참조 선택의 관찰 모드만 지원하며 기본 선택을 대체하지 않는다. 설정이 없거나 실행에 실패하면 기본 경로를 유지한다.

수동 symlink 예시는 핵심 4개만 연결한다. 선택형 연결 skill은 위 공식 CLI로 추가할 수 있다.
...
### 2-5. 기본 경로와 선택적 판단 보조 도구

관련 참조 선택과 리뷰 지적 분류는 [선택적 판단 보조 도구의 작업 계약](skills/cadence-plan/references/decision-models.md)에 따라 위임할 수 있다. 메인 에이전트가 결과를 확인하며 도구가 없으면 기본 경로를 수행한다. 시험 운용에서 작업별 정확도와 전체 처리 비용을 측정한 뒤 실사용 여부를 정한다.
...
| 상황 | 기대 |
| --- | --- |
| 도구 없음 또는 비활성 | 호출·설치 요구 없이 기본 Cadence 수행 |
| 연결 skill만 설치되고 runtime·모델은 미준비 | 연결 skill의 진단에서 미준비를 확인하고 기본 경로 수행. 설치를 연결 성공으로 보고하지 않음 |
| 단순 경로 검사나 작은 단발 작업 | 기존 결정론적 검사·직접 판단 사용 |
| 참조 추천에서 필수 규칙이 누락됨 | 필수 규칙은 유지하고 ID·관련성을 확인한 후 참조 로드 |
| 서로 다른 리뷰 지적을 중복으로 추천함 | 원본을 보존하고 코드·계약 대조로 병합 여부 결정 |
| 정보 부족·timeout·잘못된 ID·분류 반환 | 메인 에이전트가 기본 경로로 처리. 새 사용자 결정이 없다면 게이트 없음 |
| 승인·공개 계약·최종 수용을 대신 결정하려 함 | 추천을 권한이나 검증 증거로 사용하지 않음 |
| 같은 작업 계약을 지원하는 새 도구 추가 | 해당 작업·언어 평가 후 환경 설정만 추가. 공통 정책에 제품명 추가 없음 |
| 도구가 confidence 점수를 제공하지 않음 | 사전 평가·출력 검사·정보 부족 처리로 적합성 판단 |
| 시험 운용에서 정확하지만 총 비용 증가 | 기본 경로 유지 |

현재 Cadence, 절차를 간소화한 Cadence, 간소화한 Cadence + 판단 보조 도구를 같은 사례로 비교한다. 추천과 메인 판단의 일치를 정답으로 간주하지 않고 사전 기준으로 판정한다. 실제 행동 실행과 비용 측정을 하지 않았다면 미실행으로 기록한다.
...
| "도구 없음 → 기본 Cadence" | 선택 기능의 정상 기본 경로 |
...
| 보조 도구 부재로 작업 중단 | 선택 기능과 필수 의존성 혼동 | 기본 Cadence 경로 수행 |
| adapter 추천을 승인·최종 수용으로 해석 | advisory와 authority 경계 누락 | hard guardrail과 사용자 gate를 adapter 밖에서 적용 |
...
**Q. 별도 판단 보조 도구가 없어도 사용할 수 있나요?**

A. 네. 기본 Cadence가 그대로 작동하며, 평가된 판단 보조 도구가 있을 때 선택적으로 활용합니다.
