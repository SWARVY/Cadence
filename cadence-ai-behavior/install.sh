...
if [[ -f "$INDEX" ]]; then
  if grep -q '<!-- cadence-ai-behavior start -->' "$INDEX"; then
    if [[ $DRY -eq 0 ]]; then
      awk -v frag="$INDEX_FRAGMENT" '
        BEGIN { in_block=0 }
        /<!-- cadence-ai-behavior start -->/ { in_block=1; while ((getline line < frag) > 0) print line; close(frag); next }
        /<!-- cadence-ai-behavior end -->/   { in_block=0; next }
        !in_block { print }
      ' "$INDEX" > "$INDEX.tmp" && mv "$INDEX.tmp" "$INDEX"
    fi
    echo ">> MEMORY.md: cadence-ai-behavior block updated in-place"
  else
    if [[ $DRY -eq 0 ]]; then
      echo "" >> "$INDEX"
      cat "$INDEX_FRAGMENT" >> "$INDEX"
    fi
    echo ">> MEMORY.md: cadence-ai-behavior block appended"
  fi
else
  if [[ $DRY -eq 0 ]]; then
    cp "$INDEX_FRAGMENT" "$INDEX"
  fi
  echo ">> MEMORY.md: created with cadence-ai-behavior block"
fi
