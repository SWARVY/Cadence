...
description: 개인 회고 작성·관리 skill. 작업 완료 / PR merge·머지 직후 / 실패 / mid-PR 학습 / 룰 위반 발견 시 트리거. 회고 파일 생성은 사용자 합의 후지만, 회고 가치 평가(낮음/중간/높음)는 mandatory 로 먼저 출력한다. "머지하고 다음 작업 시작" 같은 연속 지시에서도 merge 와 다음 작업 사이에 평가 게이트를 끼운다. 한 줄 takeaway INDEX + 본문 (무엇/왜/해결/예방) 메타-구조. recurring 패턴은 룰화 승급 검토.
---
...
| **중간** | 다음 작업에서 참고할 판단, UX/API 방향 조정, 작은 workflow gap 발견 | 회고 후보 제목 1개 제안. 사용자가 동의하면 초안 작성 |
| **높음** | 반복 redirect, 런타임/도구 함정, 룰 위반, 큰 방향 전환, 사용자 신뢰 저하 | 회고 초안 제안 + 룰화 승급 layer 후보까지 보고 |
