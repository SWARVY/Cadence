---
name: cadence-plan
description: 결정 위험이 높거나 실행 범위가 크고, 신규 spec·도메인, 모호한 요구, 기존 시스템 제약, 추상화 판단을 검토할 때 사용한다. 기능의 수용 시나리오·검증 방법을 정하거나 테스트 변경·완료 근거를 판단할 때는 검증 절을 적용한다.
---
...
**전체 체크 제외 대상**: 명백한 typo, 동작에 영향 없는 미세 조정. 코드 줄 수만으로 위험을 판정하지 않는다. 수용 기준·테스트 의미·완료 근거를 판단하는 작업은 [§4](#4-외부-검증--사다리-verification-ladder)만 필요한 깊이로 적용할 수 있다.
...
- 주요 기능의 핵심 수용 시나리오·검증 범위가 미합의이고 명시적 위임도 없음 — 초안을 준비해 `using-cadence` §1-8에 따라 검토
...
관련 참조 선택이나 리뷰 지적 분류에 판단 보조 도구를 활용·평가할 때 [작업 계약과 도입 기준](references/decision-models.md)을 읽는다. 도구는 선택 기능이며, 없으면 메인 에이전트가 기본 Cadence를 수행한다. 사용 여부는 작업별 정확도와 입력 준비·호출·결과 확인을 포함한 전체 비용으로 정한다. 승인·공개 계약·최종 수용 경계는 기존 규칙을 따른다.
...
먼저 수용 조건에 필요한 증거를 정하고, 추가적인 판단 검토는 비용 사다리로 조절한다. **동작·영속성·렌더링 검증은 build나 AI 리뷰 실패 후에만 수행하는 상위 단계가 아니다.**

### 수용 시나리오와 증거

1. **구현 전에 핵심 시나리오를 함께 검토한다.** AI가 주요 정상 흐름, 실패·복구, 중요한 경계를 조건·행동·관찰 결과와 검증 범위로 정리하고 현행 명세·합의에 연결한다. 주요 기능은 이 초안을 사용자와 검토한 뒤 구현한다. 이미 해당 시나리오를 합의했거나 사용자가 명시적으로 위임한 범위는 반복 확인하지 않는다. 검토 경계는 [using-cadence §1-8](../using-cadence/SKILL.md#1-8-수용-기준과-검증-경계)을 따른다.
2. **수용 조건에 맞는 검증을 선택한다.** 보호할 계약·관찰할 결과·필요한 실제 의존성을 먼저 정하고, 이를 확인할 수 있는 방법 중 실행·유지보수 비용이 적은 경계를 고른다. 기능명으로 테스트 계층을 고정하거나 대체한 의존성의 동작까지 검증했다고 하지 않는다.
3. **요구와 결함을 연결하는 테스트를 작성한다.** 관찰값·기대값이 현행 계약을 확인하고 그 계약을 어기는 동작에서 실패하는지 검토한다. assertion 문법이나 정적·동적 검사 여부만으로 유용성을 판정하지 않는다. 불필요한 검사를 정리할 때 필요한 검증 공백을 메우고, 명시된 출력 자체가 계약이면 그 검사를 보존한다. 구현에서 기대값을 역산하거나 실패를 없애려고 assertion·snapshot·skip을 바꾸지 않으며, 합의된 요구 변경은 근거와 함께 반영한다.
4. **검증력과 실행 결과를 확인한다.** 버그 재현 테스트는 해당 결함에서 실패하고 수정 후 통과하는지 확인한다. 완료 보고는 실제 통과·실패·미실행과 환경 차단 이유, 남은 확인을 구분한다. 확인하지 않은 수용 조건까지 완료로 보고하지 않는다.

시나리오 설계, 의미 있는 assertion, 의존성·책임 경계, 실행 환경의 관찰, 테스트 배경 주석과 기대 결과 변경을 판단할 때 [상세 가이드](references/verification.md)를 읽는다. 작은 변경은 기존 계획·테스트 안에서 축약한다. 별도 문서, 모든 작업의 TDD/E2E, 케이스별 승인, 추가 AI reviewer를 일괄 강제하지 않는다.

### 추가 검토의 비용 사다리

Eugene Yan의 *저렴 → 비싼* 사다리 패턴을 추가 검토 비용에 적용한다. 아래 사다리는 수용 조건별 검증 방법을 대신하지 않는다.
...
| --- | --- | --- | --- |
| **L1. 결정론 (mechanical)** | 주로 실행 비용 | **자동** (post-edit hook 권장) | 관련 자동 테스트 / typecheck / lint / format / build |
...
- **L1 실패를 L2 승인으로 덮지 않는다.** 구현 결함은 범위 안에서 진단·수정한다. 환경 차단은 미실행으로 기록하고 독립적으로 가능한 검증을 이어간다. 원인 진단을 위한 도움은 완료 판정용 리뷰와 구분한다.
- **L2 가 OK 면 L3 호출 X** — *비용 효율*. L3 는 *L2 가 disagree* 또는 *사용자 명시 요청* 시만
- **L4 는 *판단 필요* 한 결정** — 도메인 추상화 / 새 컴포넌트 / 새 라우트 / contract gap 의심 시
- **각 layer는 필요한 범위에서 순차 적용한다.** 한 layer의 통과가 미실행한 검증을 대신하지 않으며, 필요한 수용 조건이 미확인이면 전체 검증 완료로 닫지 않는다.
...
이 표는 추가 검토 깊이를 정한다. 수용 조건에 필요한 동작·계약·렌더링 증거는 위험과 실행 범위에 맞춰 별도로 확보한다. [feedback_crosscheck](../cadence-ai-behavior/rules/feedback_crosscheck.md)의 독립 리뷰도 그 실행 증거를 대체하지 않는다.

## 구현 전 테스트 가능성 점검

구현 단위의 책임과 의존성을 정할 때 적용한다. 테스트 작성 시점까지 미루지 않는다.

- 보호할 동작과 입력·출력, 상태 소유권, 외부 부작용을 먼저 확인한다. 독립적인 정책 계산과 I/O·UI 조합이 한곳에 섞이면 책임 경계를 검토한다.
- 많은 대체 의존성이 필요하면 책임이 과도한지, 테스트 경계가 지나치게 좁은지, 수용 범위 밖 조건을 정당하게 통제하는지 함께 검토한다. mock 개수나 라이브러리 종류만으로 결함을 단정하지 않는다.
- 필요한 경계만 분리하고 공개 동작으로 검증한다. 작은 단위의 기계적 계층 분리, 테스트 전용 public API, 불필요한 DI·추상화를 만들지 않는다.
- 연결이나 실행 환경이 수용 조건에 포함되면 그 경계를 실제로 관찰할 방법을 선택한다. 기존 증거로 부족한 연결을 보강하고, 충분한 검증이나 세부 조합을 불필요하게 중복하지 않는다.

이 체크는 설계 시 판단 기준이다. 별도 승인 게이트나 정해진 구현 단위의 크기를 만들지 않는다. 구체적인 추출·의존성 제어·실행 설정은 프로젝트/stack 컨벤션을 따른다. [검증 경계 선택](references/verification.md#검증-경계와-의존성을-정한다)
...
| **4단. 외부 검증** | `## 수용 시나리오 / 검증 방법`, `## 검증 결과` (실행 증거·미확인 범위, 필요 시 보조 리뷰), `## 구현 체크리스트` |
...
## 수용 시나리오 / 검증 방법
- <시나리오 식별자 / 요구 근거 / 조건·행동 → 관찰 결과 / 검증 경계>
- <기존 명세·테스트에 있으면 중복 작성 대신 링크>
...
## 구현 체크리스트
- [ ] <구현 및 필요한 수용 조건 검증 완료 시 status: done>

## 검증 결과
- <시나리오 / 실제 실행 명령·환경 또는 조작 / 통과·실패·미실행 / 증거>
- <미실행 이유·남은 확인, 필요 시 보조 리뷰 의견>
