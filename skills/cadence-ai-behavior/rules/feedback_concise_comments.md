---
name: 주석은 간결하게
description: 장황한 주석과 자명한 코드 재서술 회피 — 짧은 의도 설명, 필요한 테스트 배경은 간결한 블록으로 보존
type: feedback
---
코드 주석은 기본적으로 **핵심만 한두 줄**로 작성한다. 다단 설명, 단계별 나열, 자명한 코드를 그대로 한국어로 옮겨 적는 주석은 피한다.
...
3. 컨벤션·라이브러리 동작은 *비-자명한* 동작만 한 줄로 메모.
4. 제목/섹션 구분용 주석 (`// === SECTION ===`) 은 실제 구분 가치가 클 때만.
5. 테스트 이름만으로 알 수 없는 배경·결정 이유·검증 경계는 짧은 `[테스트 배경]` 블록으로 남길 수 있다. 필요한 맥락을 줄 수에 맞춰 지우지 않는다. 공통 배경은 묶음 위에 한 번만 적고, 확인한 근거와 함께 갱신한다. 구체적 형식과 생략 기준은 [테스트 배경 주석](../../cadence-plan/references/verification.md#테스트-배경-주석)을 따른다.
