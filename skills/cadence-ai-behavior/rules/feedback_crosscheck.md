...
기능 구현이 완료되고 검증(브라우저/테스트 등)까지 끝나면, 사용자에게 결과를 보고하기 전에 **독립된 보조 리뷰 경로** 로 크로스 체크할 필요가 있는지 검토한다. 보조 리뷰 경로가 프로젝트에 설정되어 있어도 결정 위험과 검증 비용에 비례해 실행하며, 사용자가 명시적으로 "크로스 체크" 를 요청하면 수행한다.
