...
9. 주요 기능의 핵심 수용 시나리오와 검증 범위를 아직 함께 검토하지 않았고, 사용자의 명시적 위임도 없다. 구체적인 초안을 준비한 뒤 [§1-8](#1-8-수용-기준과-검증-경계)에 따라 함께 결정한다.
...
주요 기능 구현·버그 수정 전에는 [cadence-plan의 수용 시나리오와 증거](../cadence-plan/SKILL.md#수용-시나리오와-증거)를 필요한 깊이로 적용한다. 주요 기능은 새 사용자 여정이나 여러 상태·시스템을 연결하는 기능을 뜻하며, 파일 수만으로 정하지 않는다.

주요 기능은 구현 전에 핵심 정상·실패·중요 경계의 조건·행동·기대 결과와 검증 범위를 초안으로 제시하고 사용자와 묶어서 검토한다. 사람은 기대 결과, 빠진 중요한 상황, 수용 범위를 확인한다. AI가 기획을 명확하다고 판단하거나 일반적인 구현 승인을 받았다는 이유만으로 이 검토를 생략하지 않는다.

이미 해당 시나리오와 범위를 함께 합의했거나 사용자가 시나리오 선정·상세화를 명시적으로 위임했다면 그 근거와 범위 안에서 계속 진행한다. 합의된 명세에 해당 내용이 포함돼 있으면 다시 문서화하거나 승인받지 않는다. 합의 후 세부 경계값·테스트 구조는 AI가 상세화하고, 기대 동작이나 수용 범위가 달라질 때만 다시 논의한다. 위임도 기존 승인·공개 계약 경계를 넓히지 않는다.

구현 단위의 책임과 의존성을 정하기 전에는 [cadence-plan의 테스트 가능성 점검](../cadence-plan/SKILL.md#구현-전-테스트-가능성-점검)을 적용한다. 테스트하기 어렵다는 이유로 내부를 전부 mock하거나 작은 단위까지 기계적으로 분리하지 않는다.

검증 방법은 기능명·도구명·assertion 문법이 아닌 현행 계약과 필요한 실제 의존성으로 선택한다. 사례의 정책이나 검증 방법을 다른 작업에 그대로 적용하지 않는다.

테스트 기대 결과 변경이나 완료 보고 전에도 해당 기준을 확인한다. build·AI 리뷰 통과는 동작·영속성·렌더링 증거를 대체하지 않는다. 실제 통과·실패·미실행과 남은 확인을 구분하고, 합의된 요구 변경이나 의미를 보존하는 테스트 정리에 새 승인 절차를 만들지 않는다. 작은 변경에 전체 계획이나 새 테스트 체계를 요구하지 않는다.
